...
export default async function handler(req, res) {
  ...
  console.log('CSRF validation - All cookies:', Object.keys(req.cookies));
  
  if (!csrfToken) {
    console.log('CSRF token missing from request');
    return res.status(400).json({ ok: false, error: 'CSRF token is required' });
  }
  
  if (!sessionId) {
    console.log('Session ID missing from cookies');
    return res.status(400).json({ ok: false, error: 'Session not found. Please refresh the page.' });
  }
  
  if (!validateCSRFToken(csrfToken, sessionId)) {
    console.log('CSRF token validation failed');
    console.log('Token:', csrfToken.substring(0, 8) + '...');
    console.log('Session:', sessionId.substring(0, 8) + '...');
    return res.status(403).json({ ok: false, error: 'Invalid CSRF token. Please refresh the page and try again.' });
  }
